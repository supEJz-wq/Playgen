...
const emptyCode = '// Build your Playwright test, select a language and architecture, then click Generate.'

export default function PlaywrightGenerator() {
  ...
  const colors = frameworkColors.playwright

  const handleGenerate = useCallback(() => {
    const model = toTestModel(projectInfo, { framework: 'playwright', language, architecture }, steps, assertions, testData)
  ...
  const handleTemplateSelect = useCallback((template) => {
    ...
    setSteps((template.steps || []).map((s) => ({
      action: s.action, locatorType: s.locatorType || 'CSS Selector',
    ...
    setAssertions((template.assertions || []).map((a) => ({
      type: a.type, locatorType: a.locatorType || 'CSS Selector',
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Build professional Playwright automation projects with POM, reusable components, and best practices.
            </p>
          </div>
              ...
              {sections.map((sec, i) => (
                <div key={sec.id} className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className={'px-2.5 py-1 text-[11px] font-medium rounded-md transition-all duration-200 cursor-pointer whitespace-nowrap ' + (activeSection === sec.id ? 'bg-pink-500 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700')}
                  >
                    {i + 1}. {sec.label}
                  </button>
                  {i < sections.length - 1 && (
              ...
              {activeSection === 'template' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] text-pink-500 dark:text-pink-400 bg-pink-50 dark:bg-pink-900/20 px-2 py-0.5 rounded-full">Playwright</span>
                  </div>
                  <TemplateSelector onSelect={handleTemplateSelect} />
                </div>
              )}
              {activeSection === 'language' && (
                <LanguageSelector framework="playwright" value={language} onChange={setLanguage} colors={colors} />
              )}
              {activeSection === 'architecture' && (
                <ArchitectureSelector framework="playwright" language={language} value={architecture} onChange={setArchitecture} />
              )}

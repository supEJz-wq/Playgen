...
export default function ProjectInfo({ projectInfo, onChange }) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select value={projectInfo.environment || ''} onChange={(e) => update('environment', e.target.value)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-200 dark:focus:ring-pink-800 transition-colors">
        ...
        <div>
          ...
          <select value={projectInfo.priority || 'Medium'} onChange={(e) => update('priority', e.target.value)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-200 dark:focus:ring-pink-800 transition-colors">
          ...
          {tags.map((tag) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-medium border transition-all duration-200 cursor-pointer ${
                  active
                    ? 'bg-pink-600 text-white border-pink-600'
                    : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-pink-300 dark:hover:border-pink-700'
                }`}
              >
                {tag}
              </button>

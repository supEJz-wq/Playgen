...
export default function AssertionBuilder({ assertions, setAssertions }) {
  const add = () => {
    setAssertions((prev) => [...prev, { type: 'Visible', locatorType: 'CSS', locator: '', value: '' }])
  }
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        ...
        {assertions.map((a, i) => {
          const def = assertionTypes.find((at) => at.value === a.type)
          return (
            <div key={i} className="flex items-start gap-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/30 p-3">
              <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-2">
                <select value={a.type} onChange={(e) => update(i, 'type', e.target.value)} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 py-1.5 text-xs text-slate-700 dark:text-slate-300 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-200 dark:focus:ring-pink-800 transition-colors">
                ...
                {def?.needsLocator && (
                  <>
                    <select value={a.locatorType || 'CSS'} onChange={(e) => update(i, 'locatorType', e.target.value)} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 py-1.5 text-xs text-slate-700 dark:text-slate-300 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-200 dark:focus:ring-pink-800 transition-colors">
                      {locatorTypeForAssertions.map((lt) => (
                        <option key={lt} value={lt}>{lt}</option>
                      ))}
                    </select>
                    <input type="text" value={a.locator || ''} onChange={(e) => update(i, 'locator', e.target.value)} placeholder="Locator" className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 py-1.5 text-xs text-slate-700 dark:text-slate-300 placeholder-slate-400 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-200 dark:focus:ring-pink-800 transition-colors font-mono" />
                  </>
                )}
                {def?.needsValue && (
                ...
                {!def?.needsLocator && def?.needsValue && (
                  <input type="text" value={a.value || ''} onChange={(e) => update(i, 'value', e.target.value)} placeholder="Expected value" className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 py-1.5 text-xs text-slate-700 dark:text-slate-300 placeholder-slate-400 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-200 dark:focus:ring-pink-800 transition-colors" />
                )}
              </div>
